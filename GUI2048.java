 ...
 * 
 * Connor O'Neil - connoroneil Eli Jordan - ejordan3 Derek Hoshaw - dthoshaw AJ
 * Becerra - ajbecerra
 */
public class GUI2048 {
	...
	/**
	 * Starts a new game of 2048 by prompting for a grid size selection, game mode
	 * selection, then starts the game
	 */
	private void startNewGame() {
		// Prompt for grid size
		String[] gridSizes = { "4", "5", "6" };
		String gridSizeStr = (String) JOptionPane.showInputDialog(frame, "Select Grid Size:", "New Game",
				JOptionPane.QUESTION_MESSAGE, null, gridSizes, gridSizes[0]);
		...
		String gameMode = (String) JOptionPane.showInputDialog(frame, "Select Game Mode:", "New Game",
	...
	/**
	 * Opens the settings dialog box and allows for changes to be made to the theme
	 * and volume
	 */
	...
	private void endGame() {
		...
		int choice = JOptionPane.showConfirmDialog(frame, "Would you like to start a new game?", "Game Over",
	...
	private void useInput(int keyCode) {
		...
		if (validMove) {
			...
			if (!game.hasMoves()) {
						...
						public void windowClosed(WindowEvent e) {
							if (mineGame.getHasWon()) {
								...
								JOptionPane.showMessageDialog(frame,
										"You earned a second chance! The board has been randomized.");
							} else {
				...
				int choice = JOptionPane.showConfirmDialog(frame,
						"No more moves left! Would you like to start a new game?", "Game Over",
						JOptionPane.YES_NO_OPTION);
				if (choice == JOptionPane.YES_OPTION) {
					startNewGame();
				}
		...
		if (!game.hasMoves() && game.getSecLife()) {
			// Game over if second chance was already used
			stopBackground();
			String name = JOptionPane.showInputDialog(frame,
					"Game Over! Your score is: " + game.getScore() + "\nEnter your name:");
